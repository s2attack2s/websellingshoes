...
<%@ Import Namespace="TNOL.Ckeditor.Models" %>
<%@ Import Namespace="TNOL.Ckeditor.Models.Schema" %>
...
<script runat="server">
    ...
    public override bool CheckAuthentication()
    {
        try
        {
            ...
            {
                token = Common.Base64Decode(HttpUtility.UrlDecode(token));
            }
            Account tk = Common.GetAccount(token);
            if (tk == null)
            {
                return false;
            }
            if (tk.GroupId == Common.DEVELOPER)
            {
                return true;
            }
            if (!Common.CheckAuthentication(new string[] { "QUESTION_UPDATE","QUESTION_SAVE","TUTORIAL_UPDATE","TUTORIAL_SAVE"}))
            {
                return false;
            }
            return true;
    ...
    public override void SetConfig()
    {
        ...
        LicenseName = "ngocquy";
        LicenseKey = "A3KJTG9XCM9LFWQHDABUUTV5W82KY395";

        // The base URL used to reach files in CKFinder through the browser.
        BaseUrl = "/public/download/";
        ...
        type.MaxSize = 0;
        type.AllowedExtensions = new string[] { "txt", "7z", "aiff", "asf", "avi", "bmp", "csv", "doc", "docx", "fla", "flv", "gif", "gz", "gzip", "jpeg", "jpg", "mid", "mov", "mp3", "mp4", "mpc", "mpeg", "mpg", "ods", "odt", "pdf", "png", "ppt", "pptx", "pxd", "qt", "ram", "rar", "rm", "rmi", "rmvb", "rtf", "sdc", "sitd", "swf", "sxc", "sxw", "tar", "tgz", "tif", "tiff", "txt", "vsd", "wav", "wma", "wmv", "xls", "xlsx", "zip" };
